...
def get_revenue_by_time(
    ...
    for row in grouped:
        ...
        # Late-fee share of this group's own revenue (ready-made ratio so
        # the agent does not have to compute it and can cite it directly).
        row["late_fee_contribution_pct"] = pct(
            row["late_fee_revenue"], row["revenue"]
        )
    ...
    result = {
        ...
        "late_fee_contribution_pct": pct(total_late_fee, total_revenue),
        "rental_revenue_pct": pct(total_revenue - total_late_fee, total_revenue),
